  ...
  return (
    <div>
      <ToolHeader
        ...
        title="File Patcher"
        description="Write generated code directly to a file in your local project. Safe — only paths in PATCH_ALLOWED_ROOTS are allowed."
      />
      ...
      <div className="card bg-amber-50/50 border-amber-200 mb-6">
        <div className="flex gap-3">
          ...
          <div className="text-sm">
            <p className="font-medium text-ink mb-1">Setup required</p>
            <p className="text-muted mb-2">Edit <code className="bg-surface px-1 rounded">.env.local</code>:</p>
            ...
            <p className="text-muted mt-2 text-xs">Comma-separate multiple roots. Restart <code>npm run dev</code>.</p>
          </div>
        </div>
      </div>
      ...
      <div className="card space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted mt-2">
            <strong>create</strong>: fails if exists · <strong>overwrite</strong>: creates .backup.&lt;ts&gt; first · <strong>append</strong>: adds to end
          </p>
        </div>
        <div>
          <label className="text-eyebrow block mb-2">Content ({content.length} chars)</label>
          <textarea value={content} onChange={e=>setContent(e.target.value)} rows={14}
            placeholder="Paste code from Schema Builder / OG Image Kit / Rewriter…"
        ...
        {result && (
          <div className="flex gap-2 p-3 bg-green-50 border border-green-200 rounded-lg text-sm">
            ...
            <div className="text-success">
              <p className="font-medium">File written</p>
