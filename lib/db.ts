...
export function createAudit(id: string, url: string) {
  const now = Date.now();
  db.prepare(
    `INSERT INTO audits (id, url, status, created_at, updated_at, logs) VALUES (?, ?, 'pending', ?, ?, '[]')`
  ).run(id, url, now, now);
}

export function updateAuditStatus(id: string, status: AuditStatus) {
  db.prepare(`UPDATE audits SET status = ?, updated_at = ? WHERE id = ?`).run(
    status,
    Date.now(),
    id
  );
...
export function saveReport(id: string, report: unknown) {
  db.prepare(
    `UPDATE audits SET report = ?, status = 'completed', updated_at = ? WHERE id = ?`
  ).run(JSON.stringify(report), Date.now(), id);
}

export function saveError(id: string, error: string) {
  db.prepare(
    `UPDATE audits SET error = ?, status = 'failed', updated_at = ? WHERE id = ?`
